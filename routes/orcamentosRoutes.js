...
module.exports = function(db) {
  ...
  function parseMultipart(buffer, contentType) {
    ...
    while (pos >= 0) {
      let next = buffer.indexOf(marker, pos + marker.length);
      if (next < 0) break;
  ...
  router.post('/:id/importar-sintetico-excel', express.raw({ type: () => true, limit: '30mb' }), (req, res) => {
    let uploadData;
    try {
      uploadData = parseMultipart(req.body, req.headers['content-type']);
    } catch (err) {
      return res.status(400).json({ erro: err.message });
    }
    const modo = String(uploadData.fields?.modo_merge || 'substituir');
    const file = uploadData.file;
    if (!file?.buffer) return res.status(400).json({ erro: 'Arquivo Excel não enviado.' });
    let parsed;
    try {
            ...
            if (idx >= itensNormalizados.length) {
              return db.all('SELECT * FROM orcamento_sintetico WHERE id_orcamento=? ORDER BY ordem, id_item', [req.params.id], (listErr, rows) => {
                ...
                return res.json({
                  ...
                  itens: rows || [],
                  titulo_detectado: file.originalname,
