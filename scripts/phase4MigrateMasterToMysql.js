...
function mapRows(data) {
  return {
    ...
    admin_audit_log: data.admin_audit_log.map(row => ({
      id_audit_log: row.id_audit_log || row.id_log,
      id_admin_user: row.id_admin_user || row.id_admin || null,
      id_tenant: row.id_tenant || null,
      action: row.action || row.acao || 'unknown',
      entity_type: row.entity_type || row.entidade_tipo || 'unknown',
      entity_id: row.entity_id || row.entidade_id || '',
      details_json: JSON.stringify({
        admin_email: row.admin_email || null,
        before: parseJson(row.antes),
        after: parseJson(row.depois),
        original: row.details_json ? parseJson(row.details_json) : null,
      }),
      created_at: normalizeDate(row.created_at),
    })),
  };
