...
async function ensureFonte(db) {
  const row = await one(db, "SELECT id_fonte FROM fontes_referencia WHERE nome_fonte='Compras Governamentais'");
  ...
  const result = await run(
    ...
    [
      ...
      'Nacional',
      'Fonte criada automaticamente pelo modulo Pesquisa em Compras Governamentais.',
    ],
  );
  return result.lastID;
}
...
async function createInsumoFromCompra(db, data) {
