...
const RiscosContingencia = {
  ...
  renderTornado() {
    ...
    this.panel('Valor Esperado e Diagrama de Tornado','Analise preliminar e sensibilidade univariada, mantendo as demais variaveis no valor-base.',`
      <div class="risk-kpis six"><div class="risk-kpi"><span class="label">Base de calculo</span><strong>${this.money(vme.base_calculo)}</strong></div><div class="risk-kpi"><span class="label">Contingencia por VME</span><strong>${this.money(vme.contingencia_total)}</strong></div><div class="risk-kpi primary"><span class="label">Taxa por VME</span><strong>${this.pct(vme.taxa_contingencia,4)}</strong></div><div class="risk-kpi highlight"><span class="label">Contingencia por RMS</span><strong>${this.money(vme.contingencia_rms)}</strong></div><div class="risk-kpi highlight"><span class="label">Taxa por RMS</span><strong>${this.pct(vme.taxa_rms,4)}</strong></div><div class="risk-kpi"><span class="label">Variaveis</span><strong>${tornado.rows.length}</strong></div></div>
      <div class="risk-alert info"><span>ℹ</span><div><b>RMS dos valores esperados:</b> √(Σ VEᵢ² / n), considerando somente as variáveis incluídas na contingência.</div></div>
